    ...
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", flexDirection: "column", position: "relative", overflow: "hidden", transition: "background .3s" }}>
      ...
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: isMobile ? "16px 20px" : "20px 60px", position: "relative", zIndex: 10, borderBottom: `1px solid ${C.border}`, backdropFilter: "blur(8px)", background: isDark ? "rgba(2,12,24,0.6)" : "rgba(240,244,248,0.85)", transition: "background .3s" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: isMobile ? 6 : 10 }}>
          {/* Dark/Light toggle */}
          <ThemeSwitch />
          {!isMobile && <Btn variant="navy" style={{ padding: "9px 16px", fontSize: 12, borderRadius: 50 }} onClick={() => goTo("subscription")}>💳 Subscribe</Btn>}
...
/* ══════════════════════════════════════════
   HOME PAGES
══════════════════════════════════════════ */
function HomeCard({ profile, onGoToDash, onLogout, onSubscribe }) {
    ...
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden", transition: "background .3s" }}>
      ...
      <div style={{ background: C.bgCard, borderRadius: 24, padding: "clamp(24px,5vw,44px) clamp(20px,6vw,48px)", maxWidth: 480, width: "94%", position: "relative", zIndex: 2, textAlign: "center", border: `1px solid ${C.border}`, boxShadow: isDark ? "0 20px 60px rgba(0,0,0,.6)" : "0 20px 60px rgba(0,0,0,.15)", transition: "background .3s" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <button onClick={onSubscribe} style={{ background: `linear-gradient(135deg,${C.orange},#e07b1a)`, border: "none", borderRadius: 10, padding: 11, color: "#fff", fontFamily: "'Montserrat',sans-serif", fontSize: 12, fontWeight: 700, cursor: "pointer", marginTop: 0 }}>💳 Manage Subscription</button>
        </div>
      </div>
    </div>
  );
...
function InstructorDash({ profile, onHome }) {
  ...
  return (
    <Shell accent={C.sky} navItemsFlat={flatNav} sidebarItems={navItems.map((n, i) => n[0]?.section ? <SectionLabel key={i}>{n[0].section}</SectionLabel> : <NavItem key={i} icon={n[0].icon} label={n[0].label} active={page === n[0].id} onClick={() => setPage(n[0].id)} accent={C.sky} />)} user={profile} onHome={onHome} pageTitle={titles[page] || "Dashboard"} topRight={<span style={{ fontSize: 12, color: C.textMuted }}>{profile.subject}</span>}>
      ...
      {page === "students" && <StudentManagement profile={profile} students={students} onRefresh={() => supabase.from("profiles").select("*").eq("role","student").order("full_name").then(({data})=>setStudents(data||[]))} />}
      {page === "sessions" && <div>
...
/* ══════════════════════════════════════════
   STUDENT MANAGEMENT — level-up + comments
══════════════════════════════════════════ */
function StudentManagement({ profile, students, onRefresh }) {
  ...
  useEffect(() => {
    if (!selected) return;
    supabase.from("student_comments").select("*,profiles!student_comments_instructor_id_fkey(full_name)").eq("student_id", selected.id).order("created_at", { ascending: false }).then(({ data }) => setComments(data || []));
    supabase.from("level_up_log").select("*,profiles!level_up_log_instructor_id_fkey(full_name)").eq("student_id", selected.id).order("leveled_at", { ascending: false }).then(({ data }) => setLevelLog(data || []));
  }, [selected]);

  async function addComment() {
    if (!newComment.trim() || !selected) return;
    setSaving(true);
    const { error } = await supabase.from("student_comments").insert({ student_id: selected.id, instructor_id: profile.id, body: newComment.trim(), is_private: isPrivate });
    if (!error) {
      setNewComment("");
      supabase.from("student_comments").select("*,profiles!student_comments_instructor_id_fkey(full_name)").eq("student_id", selected.id).order("created_at", { ascending: false }).then(({ data }) => setComments(data || []));
      toast("Comment saved!");
  ...
  async function deleteComment(id) {
    await supabase.from("student_comments").delete().eq("id", id);
  ...
  async function levelUp() {
    if (!selected) return;
    setSaving(true);
    const oldLevel = selected.current_level || 1;
    const newLevel = oldLevel + 1;
    const xpGain = 500;
    const { error } = await supabase.from("profiles").update({ current_level: newLevel, total_xp: (selected.total_xp || 0) + xpGain }).eq("id", selected.id);
    if (!error) {
      await supabase.from("level_up_log").insert({ student_id: selected.id, instructor_id: profile.id, old_level: oldLevel, new_level: newLevel, reason: levelReason || null });
      await supabase.from("notifications").insert({ user_id: selected.id, title: `🎉 Level Up! You're now Level ${newLevel}!`, body: levelReason || `Congratulations! Keep it up!`, type: "levelup" });
      await supabase.from("achievements").insert({ student_id: selected.id, badge: "⭐", label: `Level ${newLevel} Achieved`, description: `Reached Level ${newLevel}` });
      setSelected(s => ({ ...s, current_level: newLevel, total_xp: (s.total_xp || 0) + xpGain }));
      supabase.from("level_up_log").select("*,profiles!level_up_log_instructor_id_fkey(full_name)").eq("student_id", selected.id).order("leveled_at", { ascending: false }).then(({ data }) => setLevelLog(data || []));
      onRefresh();
      setShowLevelModal(false);
      setLevelReason("");
      toast(`🎉 ${selected.full_name} leveled up to Level ${newLevel}!`);
    } else toast(error.message, "err");
    setSaving(false);
  }

  async function levelDown() {
    if (!selected || (selected.current_level || 1) <= 1) return;
    const oldLevel = selected.current_level || 1;
    const newLevel = oldLevel - 1;
    await supabase.from("profiles").update({ current_level: newLevel }).eq("id", selected.id);
    await supabase.from("level_up_log").insert({ student_id: selected.id, instructor_id: profile.id, old_level: oldLevel, new_level: newLevel, reason: "Manual adjustment" });
    setSelected(s => ({ ...s, current_level: newLevel }));
    onRefresh();
    toast("Level adjusted");
  }

  return (
    <div>
      <div style={{ marginBottom: 20 }}><h1 style={{ fontSize: 24, fontWeight: 800, color: C.text }}>My Students</h1><p style={{ fontSize: 13, color: C.textMuted, marginTop: 3 }}>{students.length} enrolled · click a student to manage</p></div>
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : selected ? "280px 1fr" : "1fr", gap: 16 }}>
        {/* Student List */}
        <Card>
          <CardHead title="Students" />
          <CardBody style={{ padding: "8px 0" }}>
            {students.length === 0 && <div style={{ textAlign: "center", color: C.textMuted, fontSize: 13, padding: "30px 0" }}>No students yet.</div>}
            {students.map(s => (
              <div key={s.id} onClick={() => { setSelected(s); }} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", cursor: "pointer", background: selected?.id === s.id ? `${C.orange}18` : "transparent", borderLeft: `3px solid ${selected?.id === s.id ? C.orange : "transparent"}`, transition: "all .15s", borderBottom: `1px solid ${C.border}` }}>
                <Avatar ini={initials(s.full_name || s.email || "").toUpperCase()} bg={C.navyMid} size={38} radius={10} src={s.avatar_url} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: C.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.full_name || s.email?.split("@")[0]}</div>
                  <div style={{ fontSize: 10, color: C.textMuted }}>Level {s.current_level || 1} · {s.total_xp || 0} XP</div>
                </div>
                <div style={{ fontSize: 11, fontWeight: 800, color: C.orangeLight }}>Lvl {s.current_level || 1}</div>
              </div>
            ))}
          </CardBody>
        </Card>
          ...
          <div>
            ...
            <Card style={{ marginBottom: 16 }}>
              <CardBody style={{ padding: "20px 24px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                  <Avatar ini={initials(selected.full_name || "").toUpperCase()} bg={C.navyMid} size={64} radius={16} src={selected.avatar_url} />
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 20, fontWeight: 800, color: C.text }}>{selected.full_name || "Student"}</div>
                    <div style={{ fontSize: 12, color: C.textMuted, marginTop: 3 }}>{selected.email} {selected.age ? `· Age ${selected.age}` : ""} {selected.school_name ? `· ${selected.school_name}` : ""}</div>
                    <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
                      <span style={{ padding: "4px 12px", borderRadius: 20, background: `${C.orange}22`, color: C.orangeLight, fontSize: 11, fontWeight: 700 }}>Level {selected.current_level || 1}</span>
                      <span style={{ padding: "4px 12px", borderRadius: 20, background: `${C.navyMid}22`, color: C.sky, fontSize: 11, fontWeight: 700 }}>{selected.total_xp || 0} XP</span>
                      <span style={{ padding: "4px 12px", borderRadius: 20, background: "rgba(99,102,241,.18)", color: "#a5b4fc", fontSize: 11, fontWeight: 700 }}>Year {selected.year || 1}</span>
                    </div>
                  </div>
                  {/* Level-up / Level-down buttons */}
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={() => setShowLevelModal(true)} style={{ padding: "10px 18px", borderRadius: 10, background: `linear-gradient(135deg,${C.orange},#e07b1a)`, color: "#fff", border: "none", cursor: "pointer", fontWeight: 700, fontSize: 13, fontFamily: "'Montserrat',sans-serif", boxShadow: "0 4px 14px rgba(242,147,43,0.35)" }}>⬆ Level Up</button>
                    <button onClick={levelDown} style={{ padding: "10px 18px", borderRadius: 10, background: C.bgElevated, color: C.textMuted, border: `1px solid ${C.border}`, cursor: "pointer", fontWeight: 700, fontSize: 13, fontFamily: "'Montserrat',sans-serif" }}>⬇</button>
                  </div>
                </div>
              </CardBody>
            </Card>
            ...
            {showLevelModal && (
              <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.65)", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
                <div style={{ background: C.bgCard, borderRadius: 18, padding: 28, maxWidth: 400, width: "100%", border: `1px solid ${C.border}`, boxShadow: "0 20px 60px rgba(0,0,0,.5)" }}>
                  <div style={{ fontSize: 18, fontWeight: 800, color: C.text, marginBottom: 6 }}>🎉 Level Up {selected.full_name}?</div>
                  <div style={{ fontSize: 13, color: C.textMuted, marginBottom: 18 }}>Current: Level {selected.current_level || 1} → Level {(selected.current_level || 1) + 1}<br />Student will receive +500 XP and a notification.</div>
                  <div style={{ marginBottom: 16 }}>
                    <label style={{ fontSize: 10, fontWeight: 700, color: C.textMid, letterSpacing: 1.5, textTransform: "uppercase", display: "block", marginBottom: 6 }}>Reason / Note (optional)</label>
                    <textarea value={levelReason} onChange={e => setLevelReason(e.target.value)} placeholder="e.g. Completed Level 3 project with excellence…" rows={3} style={{ width: "100%", padding: "10px 14px", borderRadius: 8, border: `1.5px solid ${C.border}`, fontFamily: "'Montserrat',sans-serif", fontSize: 13, background: C.bgSurface, color: C.text, resize: "vertical" }} />
                  </div>
                  <div style={{ display: "flex", gap: 10 }}>
                    <Btn variant="ghost" onClick={() => { setShowLevelModal(false); setLevelReason(""); }} style={{ flex: 1, borderRadius: 10 }}>Cancel</Btn>
                    <Btn variant="primary" loading={saving} onClick={levelUp} style={{ flex: 1, borderRadius: 10 }}>Confirm Level Up 🚀</Btn>
                  </div>
                </div>
              </div>
            )}
            ...
            {levelLog.length > 0 && (
              <Card style={{ marginBottom: 16 }}>
                <CardHead title="Level History" />
                <CardBody style={{ padding: "8px 0" }}>
                  {levelLog.map(l => (
                    <div key={l.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 18px", borderBottom: `1px solid ${C.border}` }}>
                      <div style={{ fontSize: 18 }}>{l.new_level > l.old_level ? "⬆️" : "⬇️"}</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 12, fontWeight: 700, color: C.text }}>Level {l.old_level} → Level {l.new_level}</div>
                        {l.reason && <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>{l.reason}</div>}
                        <div style={{ fontSize: 10, color: C.textMuted, marginTop: 2 }}>by {l.profiles?.full_name || "Instructor"} · {new Date(l.leveled_at).toLocaleDateString()}</div>
                      </div>
                    </div>
                  ))}
                </CardBody>
            ...
            {/* Comments */}
            <Card>
              <CardHead title="Instructor Notes & Comments" right={<span style={{ fontSize: 10, color: C.textMuted }}>Private by default</span>} />
              <CardBody>
                {/* Add Comment */}
                <div style={{ marginBottom: 16 }}>
                  <textarea value={newComment} onChange={e => setNewComment(e.target.value)} placeholder={`Add a note about ${selected.full_name?.split(" ")[0]}…`} rows={3} style={{ width: "100%", padding: "12px 14px", borderRadius: 8, border: `1.5px solid ${C.border}`, fontFamily: "'Montserrat',sans-serif", fontSize: 13, background: C.bgSurface, color: C.text, resize: "vertical", boxSizing: "border-box", marginBottom: 8 }} />
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
                    <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 12, color: C.textMid, fontWeight: 600 }}>
                      <input type="checkbox" checked={isPrivate} onChange={e => setIsPrivate(e.target.checked)} style={{ accentColor: C.orange, width: 15, height: 15 }} />
                      {isPrivate ? "🔒 Private (instructors only)" : "👁 Visible to parents"}
                    </label>
                    <Btn variant="navy" loading={saving} onClick={addComment} style={{ borderRadius: 9, padding: "9px 20px" }}>Save Note</Btn>
                  </div>
                </div>
                {/* Comment List */}
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {comments.length === 0 && <div style={{ textAlign: "center", color: C.textMuted, fontSize: 13, padding: "20px 0" }}>No notes yet. Add one above.</div>}
                  {comments.map(c => (
                    <div key={c.id} style={{ padding: "12px 16px", background: C.bgElevated, borderRadius: 10, border: `1px solid ${c.is_private ? C.border : "rgba(242,147,43,.25)"}`, position: "relative" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8, marginBottom: 6 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 20, background: c.is_private ? "rgba(99,102,241,.15)" : "rgba(242,147,43,.15)", color: c.is_private ? "#a5b4fc" : C.orangeLight }}>{c.is_private ? "🔒 Private" : "👁 Visible to parents"}</span>
                          <span style={{ fontSize: 10, color: C.textMuted }}>{c.profiles?.full_name || "Instructor"} · {new Date(c.created_at).toLocaleDateString()}</span>
                        </div>
                        {c.instructor_id === profile.id && <button onClick={() => deleteComment(c.id)} style={{ background: "transparent", border: "none", cursor: "pointer", color: "#f87171", fontSize: 14, padding: 0, lineHeight: 1 }}>✕</button>}
                      </div>
                      <div style={{ fontSize: 13, color: C.text, lineHeight: 1.6 }}>{c.body}</div>
                    </div>
                  ))}
                </div>
              </CardBody>
...
══════════════════════════════════════════ */
const PLANS = [
  { id: "monthly",  label: "Monthly",    price: 1200,  currency: "EGP", desc: "Pay month by month, cancel anytime",  badge: null,          icon: "📅" },
  { id: "termly",   label: "Per Term",   price: 3200,  currency: "EGP", desc: "3-month term — save 11%",            badge: "Popular",     icon: "⭐" },
  { id: "yearly",   label: "Yearly",     price: 10800, currency: "EGP", desc: "Full year — best value, save 25%",   badge: "Best Value",  icon: "🏆" },
];

function SubscriptionPage({ profile, goTo, onSubscribed }) {
  const isMobile = useIsMobile();
  const { isDark } = useTheme();
  const [form, setForm] = useState({
    ...
    school_name: "",
    student_age: profile?.age ? String(profile.age) : "",
    plan: "termly",
  });
  const [err, setErr] = useState("");
  const [ld, setLd] = useState(false);
  ...
  async function proceed() {
    setErr("");
    if (!f("student_name") || !f("parent_name") || !f("parent_phone") || !f("student_age") || !f("plan")) {
    ...
    const { data, error } = await supabase.from("subscriptions").insert({
      ...
      student_phone: f("student_phone") || null,
      parent_phone: f("parent_phone"),
      school_name: f("school_name") || null,
      student_age: parseInt(f("student_age")),
      plan: f("plan"),
    ...
    onSubscribed(data, form);
  }

  const selectedPlan = PLANS.find(p => p.id === f("plan"));

  return (
    <div style={{ minHeight: "100vh", background: C.bg, padding: isMobile ? "20px 16px" : "40px 20px", transition: "background .3s" }}>
      <MatrixRain opacity={0.08} />
      <div style={{ maxWidth: 640, margin: "0 auto", position: "relative", zIndex: 2 }}>
        {/* Back */}
        <div onClick={() => goTo("landing")} style={{ display: "inline-flex", alignItems: "center", gap: 6, cursor: "pointer", color: C.textMid, fontSize: 13, fontWeight: 600, marginBottom: 24 }}>← Back</div>

        <NexaLogo size={46} style={{ marginBottom: 28 }} />

        <div style={{ marginBottom: 32 }}>
          <h1 style={{ fontSize: isMobile ? 26 : 32, fontWeight: 900, color: C.text, lineHeight: 1.15, marginBottom: 8 }}>Join NEXA Tech School</h1>
          <p style={{ fontSize: 14, color: C.textMid, lineHeight: 1.7 }}>Choose your plan and fill in the details below. We'll confirm your enrollment within 24 hours.</p>
        </div>

        {/* Plan selector */}
        <div style={{ marginBottom: 28 }}>
          <div style={{ fontSize: 10, fontWeight: 800, color: C.textMid, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 12 }}>Select Your Plan</div>
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3,1fr)", gap: 10 }}>
            {PLANS.map(plan => (
              <div key={plan.id} onClick={() => setForm(p => ({ ...p, plan: plan.id }))} style={{ border: `2px solid ${f("plan") === plan.id ? C.orange : C.border}`, background: f("plan") === plan.id ? `${C.orange}12` : C.bgCard, borderRadius: 14, padding: "16px 14px", cursor: "pointer", transition: "all .2s", position: "relative", overflow: "hidden" }}>
                {plan.badge && <div style={{ position: "absolute", top: 8, right: 8, fontSize: 9, fontWeight: 800, padding: "2px 8px", borderRadius: 20, background: plan.badge === "Best Value" ? C.orange : C.navyMid, color: "#fff" }}>{plan.badge}</div>}
                <div style={{ fontSize: 22, marginBottom: 6 }}>{plan.icon}</div>
                <div style={{ fontSize: 14, fontWeight: 800, color: C.text, marginBottom: 3 }}>{plan.label}</div>
                <div style={{ fontSize: 18, fontWeight: 900, color: C.orange, marginBottom: 4 }}>{plan.price.toLocaleString()} {plan.currency}</div>
                <div style={{ fontSize: 10, color: C.textMuted }}>{plan.desc}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Enrollment Form */}
        <Card>
          <CardHead title="Enrollment Details" />
          <CardBody>
            {err && <div style={{ background: "rgba(239,68,68,.1)", border: "1px solid rgba(239,68,68,.25)", borderRadius: 8, padding: "10px 14px", fontSize: 12, color: "#f87171", marginBottom: 14 }}>{err}</div>}
            <FormRow>
              <FInput label="Student Full Name *" placeholder="First and last name" value={f("student_name")} onChange={set("student_name")} />
              <FInput label="Parent / Guardian Name *" placeholder="First and last name" value={f("parent_name")} onChange={set("parent_name")} />
            </FormRow>
            <FormRow>
              <FInput label="Student Phone" optional placeholder="01XXXXXXXXX" value={f("student_phone")} onChange={set("student_phone")} />
              <FInput label="Parent Phone *" placeholder="01XXXXXXXXX" value={f("parent_phone")} onChange={set("parent_phone")} />
            </FormRow>
            <FormRow>
              <FInput label="School Name" optional placeholder="e.g. Cairo American College" value={f("school_name")} onChange={set("school_name")} />
              <FSelect label="Student Age *" value={f("student_age")} onChange={set("student_age")} options={[{ value: "", label: "Select age…" }, ...Array.from({ length: 12 }, (_, i) => ({ value: String(i + 6), label: `${i + 6} years old` }))]} />
            </FormRow>

            {/* Summary */}
            {selectedPlan && (
              <div style={{ background: `${C.orange}10`, border: `1px solid ${C.orange}40`, borderRadius: 10, padding: "14px 18px", marginBottom: 18, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
                <div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: C.text }}>{selectedPlan.icon} {selectedPlan.label} Plan</div>
                  <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>{selectedPlan.desc}</div>
                </div>
                <div style={{ fontSize: 22, fontWeight: 900, color: C.orange }}>{selectedPlan.price.toLocaleString()} EGP</div>
              </div>
            )}

            <Btn variant="primary" block loading={ld} onClick={proceed} style={{ borderRadius: 12, padding: "15px 20px", fontSize: 15 }}>Continue to Payment →</Btn>
          </CardBody>
        </Card>

        <div style={{ textAlign: "center", marginTop: 20, fontSize: 11, color: C.textMuted }}>🔒 Secure enrollment · NEXA Tech School · Cairo, Egypt</div>
      </div>
    </div>
  );
}
...
/* ══════════════════════════════════════════
   PAYMENT PAGE (card UI — ready for Paymob)
══════════════════════════════════════════ */
function PaymentPage({ subscriptionData, formData, goTo, onPaid }) {
  const isMobile = useIsMobile();
  const { isDark } = useTheme();
  const [card, setCard] = useState({ number: "", name: "", expiry: "", cvv: "" });
  const [ld, setLd] = useState(false);
  const [err, setErr] = useState("");
  const plan = PLANS.find(p => p.id === subscriptionData?.plan) || PLANS[1];
  ...
  async function pay() {
    setErr("");
    if (!card.number || !card.name || !card.expiry || !card.cvv) { setErr("Please fill all card details."); return; }
    if (card.number.replace(/\s/g, "").length < 16) { setErr("Please enter a valid 16-digit card number."); return; }
    setLd(true);
    // TODO: Integrate Paymob here — send subscriptionData.id + card to Paymob API
    // For now simulate a pending payment and mark as pending in DB
    await new Promise(r => setTimeout(r, 1800));
    await supabase.from("subscriptions").update({ status: "pending", payment_method: "card" }).eq("id", subscriptionData.id);
    setLd(false);
    onPaid();
  }

  return (
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: isMobile ? "20px 16px" : "40px 20px", transition: "background .3s" }}>
      <MatrixRain opacity={0.06} />
      <div style={{ maxWidth: 480, width: "100%", position: "relative", zIndex: 2 }}>
        <div onClick={() => goTo("subscription")} style={{ display: "inline-flex", alignItems: "center", gap: 6, cursor: "pointer", color: C.textMid, fontSize: 13, fontWeight: 600, marginBottom: 24 }}>← Back</div>

        {/* Order summary */}
        <div style={{ background: `linear-gradient(135deg,${C.navy},${C.navyMid})`, borderRadius: 16, padding: "20px 24px", marginBottom: 20, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,.4)", letterSpacing: 2, textTransform: "uppercase", marginBottom: 3 }}>Order Summary</div>
            <div style={{ fontSize: 16, fontWeight: 800, color: "#fff" }}>{plan.icon} NEXA {plan.label}</div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,.5)", marginTop: 3 }}>{formData?.student_name || ""}</div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 28, fontWeight: 900, color: C.orange }}>{plan.price.toLocaleString()}</div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,.4)" }}>EGP</div>
          </div>
        </div>

        {/* Card form */}
        <Card>
          <CardHead title="💳 Card Details" />
          <CardBody>
            {err && <div style={{ background: "rgba(239,68,68,.1)", border: "1px solid rgba(239,68,68,.25)", borderRadius: 8, padding: "10px 14px", fontSize: 12, color: "#f87171", marginBottom: 14 }}>{err}</div>}

            {/* Visual card preview */}
            <div style={{ background: `linear-gradient(135deg,#0d2540,#23558a)`, borderRadius: 14, padding: "20px 22px", marginBottom: 20, minHeight: 120, position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", right: -20, top: -20, width: 120, height: 120, borderRadius: "50%", background: "rgba(242,147,43,.12)" }} />
              <div style={{ position: "absolute", right: 20, top: -40, width: 160, height: 160, borderRadius: "50%", background: "rgba(242,147,43,.06)" }} />
              <div style={{ fontSize: 10, color: "rgba(255,255,255,.4)", letterSpacing: 2, marginBottom: 16 }}>NEXA TECH SCHOOL</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: "#fff", letterSpacing: 3, marginBottom: 14, fontFamily: "'Courier New',monospace" }}>
                {card.number || "•••• •••• •••• ••••"}
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
                <div>
                  <div style={{ fontSize: 8, color: "rgba(255,255,255,.4)", letterSpacing: 1.5, marginBottom: 3 }}>CARD HOLDER</div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#fff", textTransform: "uppercase" }}>{card.name || "YOUR NAME"}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 8, color: "rgba(255,255,255,.4)", letterSpacing: 1.5, marginBottom: 3 }}>EXPIRES</div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#fff" }}>{card.expiry || "MM/YY"}</div>
                </div>
              </div>
            </div>

            <FInput label="Card Number" placeholder="1234 5678 9012 3456" value={card.number} onChange={e => setCard(c => ({ ...c, number: formatCardNumber(e.target.value) }))} />
            <FInput label="Cardholder Name" placeholder="As shown on card" value={card.name} onChange={e => setCard(c => ({ ...c, name: e.target.value.toUpperCase() }))} />
            <FormRow>
              <FInput label="Expiry Date" placeholder="MM/YY" value={card.expiry} onChange={e => setCard(c => ({ ...c, expiry: formatExpiry(e.target.value) }))} />
              <FInput label="CVV" placeholder="•••" value={card.cvv} onChange={e => setCard(c => ({ ...c, cvv: e.target.value.replace(/\D/g, "").slice(0, 4) }))} />
            </FormRow>

            <Btn variant="primary" block loading={ld} onClick={pay} style={{ borderRadius: 12, padding: "15px 20px", fontSize: 15, background: `linear-gradient(135deg,${C.orange},#e07b1a)`, boxShadow: "0 4px 20px rgba(242,147,43,.4)", marginTop: 4 }}>
              {ld ? "Processing…" : `Pay ${plan.price.toLocaleString()} EGP`}
            </Btn>
            <div style={{ textAlign: "center", marginTop: 12, fontSize: 11, color: C.textMuted }}>🔒 Payments processed securely · Paymob</div>
          </CardBody>
        </Card>
      </div>
    </div>
...
══════════════════════════════════════════ */
function PaymentSuccess({ goTo }) {
  const { isDark } = useTheme();
  return (
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, transition: "background .3s" }}>
      <MatrixRain opacity={0.08} />
      <div style={{ textAlign: "center", maxWidth: 420, position: "relative", zIndex: 2 }}>
        <div style={{ fontSize: 72, marginBottom: 16, animation: "pulseGlow 2s ease-in-out infinite" }}>🎉</div>
        <h1 style={{ fontSize: 28, fontWeight: 900, color: C.text, marginBottom: 10 }}>Enrollment Received!</h1>
        <p style={{ fontSize: 15, color: C.textMid, lineHeight: 1.7, marginBottom: 28 }}>Your subscription request has been submitted successfully.<br />Our team will confirm your enrollment and activate your account within <strong style={{ color: C.orange }}>24 hours</strong>.</p>
        <div style={{ background: C.bgCard, borderRadius: 14, padding: "18px 22px", border: `1px solid ${C.border}`, marginBottom: 28, textAlign: "left" }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: C.text, marginBottom: 8 }}>What happens next?</div>
          {["Our team reviews your enrollment","You'll receive a confirmation call","Account gets activated","Start your NEXA journey! 🚀"].map((s, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
              <div style={{ width: 22, height: 22, borderRadius: "50%", background: `${C.orange}22`, color: C.orange, fontSize: 11, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{i + 1}</div>
              <div style={{ fontSize: 12, color: C.textMid }}>{s}</div>
            </div>
          ))}
        </div>
        <Btn variant="primary" onClick={() => goTo("landing")} style={{ padding: "14px 36px", fontSize: 14, borderRadius: 50 }}>Back to Home</Btn>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  function goTo(dest) {
    ...
    else if (dest === "subscription") setScreen("subscription");
    else setScreen(dest);
  }
  ...
  return (
    <ThemeContext.Provider value={{ isDark, toggle: toggleTheme }}>
      <div style={{ fontFamily: "'Montserrat', sans-serif", background: C.bg, minHeight: "100vh", transition: "background .3s" }}>
        ...
        {screen === "subscription" && <SubscriptionPage profile={currentProfile} goTo={goTo} onSubscribed={(data, form) => { setSubscriptionData(data); setSubscriptionForm(form); setScreen("payment"); }} />}
        {screen === "payment" && subscriptionData && <PaymentPage subscriptionData={subscriptionData} formData={subscriptionForm} goTo={goTo} onPaid={() => setScreen("payment-success")} />}
        {screen === "payment-success" && <PaymentSuccess goTo={goTo} />}
        {screen === "student-home" && currentProfile && <HomeCard profile={currentProfile} onGoToDash={() => setScreen("student-dash")} onLogout={handleLogout} onSubscribe={() => setScreen("subscription")} />}
        {screen === "parent-home" && currentProfile && <HomeCard profile={currentProfile} onGoToDash={() => setScreen("parent-dash")} onLogout={handleLogout} onSubscribe={() => setScreen("subscription")} />}
        {screen === "instructor-home" && currentProfile && <HomeCard profile={currentProfile} onGoToDash={() => setScreen("instructor-dash")} onLogout={handleLogout} onSubscribe={() => setScreen("subscription")} />}
